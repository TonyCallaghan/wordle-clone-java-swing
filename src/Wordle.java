...
public class Wordle implements ActionListener {
    ...
    static Font helSmaller = new Font("Helvetica", Font.BOLD, 13); // Font for enter / backspace buttons
    static String userInput = "", answer = "";
    static int rowCount = 0; // For the grid
    static List<String> words = new ArrayList<>(); // stores possible answers
    static List<String> allowable = new ArrayList<>(); // stores acceptable words
    ...
    static Color dark = new Color(31, 31, 31); // Increased Brightness from WORDLE's dark mode
    ...
    public static void main(String[] args) throws IOException {
        Path wordsPath = Paths.get("C:/Users/Tony/wordle-clone-java-swing/src/words.txt"); // Get file with wordle-words
                                                                                           // in it
    ...
    public static void checkWord() {
        ...
        rowCount += 5;
        if (answer.equals(userInput)) { // User has WON !
            textfield.setText("WELL DONE");
            suggestionField.setText("\u21BB PLAY AGAIN \u21BB");
            chances = 6; // game over
        } else if (chances == 6) { // User has LOST !
    ...
    public static void suggestionReset() { // Refill arrayList of suggestions
    ...
    public static void kbPanel(JPanel row) { // Apply these properties to every row in the Keyboard 
    ...
    public static void findSuggestions() { 
        ...
        for (int i = 0; i < userInput.length(); i++) {
            for (int j = 0; j < suggestions.size(); j++) {
                ...
                if (greenOrangeOrGrey(i) == 0) { // letter is GREY
                    if (check.contains(String.valueOf(userInput.charAt(i)))) {
                        suggestions.remove(j);
                        j--;
                    }

                } else if (greenOrangeOrGrey(i) == 1) { // letter is ORANGE
                    if (!check.contains(String.valueOf(userInput.charAt(i)))) {
                        suggestions.remove(j);
                        j--;
                    }
                ...
                } else if (greenOrangeOrGrey(i) == 2) { // letter is GREEN
                    if (userInput.charAt(i) != check.charAt(i)) {
                        suggestions.remove(j);
                        j--;
                    }
	...
	// ALGORITHM to solve WORDLE itself based on the suggestions provided - WORKS 100% OF THE TIME
    public static void letsCheckAlgo() {          
        if(!instructions("ALIEN")) {
            if(!instructions("TOURS")) { // These 2 words contain all the vowels and top 5 most frequent consonants - so makes it easy :)
                if(suggestions.size() > 3) { // Usually can be solved before we get to this point
                    if(!instructions("NYMPH")) // If this was not included, we only have 98% solve rate - This makes it 100% :)
                        if(!instructions(suggestions.get(0).toUpperCase())) 
                            if(!instructions(suggestions.get(0).toUpperCase())) 
                                if(!instructions(suggestions.get(0).toUpperCase())) {} // if we get to this point - then suggestions have failed us :(
                } else {
                    if(!instructions(suggestions.get(0).toUpperCase()))
                        if(!instructions(suggestions.get(0).toUpperCase())) 
                            if(!instructions(suggestions.get(0).toUpperCase())) 
